...
wandb_config = {"project": "bus-agent"}
...
business_name = "Decentryk" #bus_name
industry = "Blockchain LASER verification" #industry
...
with open(filename, "r") as f:
    text = f.read()
    type(text)
stg.embed_upsert(str(text), filename, business_name, industry)
...
if user_input and industry:
    with st.spinner('Wait for it...'):
        output = bus_agent(user_input, industry)
        time.sleep(5)
    st.success('Done!')
